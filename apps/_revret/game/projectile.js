rvr.Projectile = function(index)
{
	this.index = index;
	this.reset();
}

rvr.Projectile.prototype.reset = function()
{
...
rvr.Projectile.prototype.spawn = function(
	type,
	pos_x,
	pos_y,
	pos_z,
	...
	)
{
	...
	this.is_active 				= true;
	this.last_trail 			= rvr.now - 5; // hmmm
...
rvr.Projectile.prototype.agent_collision_check = function(time_delta, agent)
{
	var agent_index = agent.index;
	if (this.faction_id != agent.parameters.faction_id)
	{
		var dist_m = µ.distance2D_manhattan(rvr_agents__pos_x[agent_index], rvr_agents__pos_y[agent_index], this.pos_x, this.pos_y);
		if (dist_m < agent.parameters.radius)
		{
			var dist = µ.distance2D(rvr_agents__pos_x[agent_index], rvr_agents__pos_y[agent_index], this.pos_x, this.pos_y);
			if (dist < agent.parameters.radius)
			{
				if (!this.bounce_agents)
				{
					...
					var angle = µ.vector2D_to_angle(rvr_agents__pos_x[agent_index] - this.pos_x, rvr_agents__pos_y[agent_index] - this.pos_y);
					rvr_agents__speed_x[agent_index] += µ.angle_to_x(angle) * 0.0002;
					rvr_agents__speed_y[agent_index] += µ.angle_to_y(angle) * 0.0002;
					this.explode();
				}
				else
				{
					var angle = µ.vector2D_to_angle(rvr_agents__pos_x[agent_index] - this.pos_x, rvr_agents__pos_y[agent_index] - this.pos_y);
					//var vector_len = µ.vector2D_length(rvr_agents__pos_x[agent_index], rvr_agents__pos_y[agent_index], this.pos_x, this.pos_y);
...
rvr.Projectile.prototype.deactivation_begin = function()
{
	this.type.explode(this); // move this to type::on_deactivation_begin
	this.is_deactivating = true;
	rvr.projectiles.count_dea++;
	this.age = 0;					// age now counts up from moment of deactivation
}

rvr.Projectile.prototype.deactivation_finish = function()
{
	rvr.projectiles.count_deac++;
	this.is_active = false;
	rvr.projectiles.active_projectiles--;
}

rvr.Projectile.prototype.think = function(time_delta)
{
	...
	this.age += time_delta;
	if (this.age >= this.lifespan || (this.pos_x < 0.0 || this.pos_x > rvr.map.size_x || this.pos_y < 0.0 || this.pos_y > rvr.map.size_y))
	{
		this.explode();
	...
	if (rvr.map.collision_check(this.pos_x + this.speed_x * time_delta, this.pos_y, this.pos_z))
	{
		if (!this.bounce)
		{
			this.explode();
	...
	if (rvr.map.collision_check(this.pos_x, this.pos_y + this.speed_y * time_delta, this.pos_z))
	{
		if (!this.bounce)
		{
			this.explode();
...
rvr.Projectile.prototype.explode = function()
{
	rvr.map.drop_smoke(this.pos_x, this.pos_y, 1);
	rvr.screen_shake = 0;
	rvr.projectiles.count_expl++;
	this.deactivation_begin();
	this.type.explode(this);
};


rvr.Projectile.prototype.draw = function()
{
	this.type.draw(this);
}


rvr.Projectile.prototype.draw_light = function()
{
//*
	var frac = 1.0;
	if (this.is_deactivating)
...
/*
			rvr.lights.add(
				this.pos_x / rvr.map.size_x,
				this.pos_y / rvr.map.size_y,
				0.0075 * frac,
				0.0075 * frac,
				0.65,
				0.95, 0.95, 0.95, 0.25,
...
*/
		...
		//	rvr.lights.add(this.pos_x / rvr.map.size_x, this.pos_y / rvr.map.size_y,		0.05 * frac, 	0.0075 * frac, 	1.0,			0.8, -.3, -.3, 0.25,	(this.age * 0.33 + 77) % 360, 	360);
		//	rvr.lights.add(this.pos_x / rvr.map.size_x, this.pos_y / rvr.map.size_y,		0.0125 * frac, 	0.005 * frac, 	1.0,			0.9, 0.6, -.4, 0.5,	(this.age * 0.37 + 177) % 360, 	360);
	}
//*/
...
rvr.Projectile.prototype._ = function()
{
}
