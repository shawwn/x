rvr.presets.agents.drone_scout =
{
	controller							: 'enemy',
	inherits_from						:
	[
	...
	draw_function 						: function(here)
	{
		...
		rvr.c.rectangle_textured.draw(
			rvr.CAM_PLAYER, this.is_dying ? rvr.tex_orb_bg_blinds1 : rvr.tex_orb_bg,
		...
		rvr.c.rectangle_textured.draw(
			...
			hue,
			1.0 * sat_factor,
			(1.25 + (this.is_dying ? 0 : Math.sin(rvr.now / 167) * Math.sin(rvr.now / 137) * 0.25)) * (this.is_dying ? 0.5 : 1.0)
				 + damage_frac * 1.5,
