...
plt.render = function()
{
	...
	var resolution = 1000;
	var bar_width = 1 / plt.events.length;
	var bar_width2 = 0.5 / plt.events.length;

	var repetitions = 10;

	var min = 9999999999999;
	var max = -9999999999999;
	for (var j = 0; j < plt.events.length; j++) 
	{
		min = Math.min(plt.events[j].start, min);
		min = Math.min(plt.events[j].end, min);
		max = Math.max(plt.events[j].start, max);
		max = Math.max(plt.events[j].end, max);
	}
	var scale = max - min;

	for (var j = 0; j < plt.events.length; j++) 
	{
		var hue = (j/plt.events.length);
		//var output = plt.events[j];
		plt.c.rectangle.draw(plt.CAM_STRETCH,
			(plt.events[j].end - plt.events[j].start) / scale / 2,
			j * bar_width + bar_width2,
			(plt.events[j].end - plt.events[j].start) / scale,
			bar_width,
			90,
			hue * 360, 1.0, 0.6, 1.0,
			-1, -1, -1, -1,
	...
	plt.fonts.draw_text(
		...
		plt.CAM_STRETCH, plt.FONT_DEFAULT, 0.05, 0.95, 0.035, 0.055, 0.01,
		0, 0, .3, .9,
		0, 0, .5, .9,
		0, 0, .99, .9,
		0, 0, .9, .9
	...
	plt.fonts.draw_text(
		...
		plt.CAM_STRETCH, plt.FONT_DEFAULT, 0.05, 0.9, 0.035, 0.055, 0.01,
		0, 0, .3, .9,
		0, 0, .5, .9,
		0, 0, .99, .9,
		0, 0, .9, .9
