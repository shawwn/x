hc.Projectile = function(index)
{
	this.index = index;
	this.reset();
}

hc.Projectile.prototype.reset = function()
{
...
hc.Projectile.prototype.spawn = function(
	type,
	pos_x,
	pos_y,
	pos_z,
...
hc.Projectile.prototype.agent_collision_check = function(time_delta, agent)
{
...
hc.Projectile.prototype.deactivation_begin = function()
{
	this.type.explode(this); // move this to type::on_deactivation_begin
	this.is_deactivating = true;
	hc.projectiles.count_dea++;
	this.age = 0;					// age now counts up from moment of deactivation
}

hc.Projectile.prototype.deactivation_finish = function()
{
	hc.projectiles.count_deac++;
	this.is_active = false;
	hc.projectiles.active_projectiles--;
}

hc.Projectile.prototype.think = function(time_delta)
{
	...
	this.age += time_delta;
	if (this.age >= this.lifespan || (this.pos_x < 0.0 || this.pos_x > hc.map.size_x || this.pos_y < 0.0 || this.pos_y > hc.map.size_y))
	{
		this.explode();
	...
	if (hc.map.collision_check(this.pos_x + this.speed_x * time_delta, this.pos_y, this.pos_z))
	{
		if (!this.bounce)
		{
			this.explode();
	...
	if (hc.map.collision_check(this.pos_x, this.pos_y + this.speed_y * time_delta, this.pos_z))
	{
		if (!this.bounce)
		{
			this.explode();
...
hc.Projectile.prototype.explode = function()
{
	hc.map.drop_smoke(this.pos_x, this.pos_y, 1);
	hc.screen_shake = 0;
	hc.projectiles.count_expl++;
	this.deactivation_begin();
	this.type.explode(this);
};


hc.Projectile.prototype.draw = function()
{
	this.type.draw(this);
}


hc.Projectile.prototype.draw_light = function()
{
//*
	var frac = 1.0;
	if (this.is_deactivating)
...
/*
			hc.lights.add(
				this.pos_x / hc.map.size_x,
				this.pos_y / hc.map.size_y,
				0.0075 * frac,
				0.0075 * frac,
				0.65,
				0.95, 0.95, 0.95, 0.25,
...
*/
		...
		//	hc.lights.add(this.pos_x / hc.map.size_x, this.pos_y / hc.map.size_y,		0.05 * frac, 	0.0075 * frac, 	1.0,			0.8, -.3, -.3, 0.25,	(this.age * 0.33 + 77) % 360, 	360);
		//	hc.lights.add(this.pos_x / hc.map.size_x, this.pos_y / hc.map.size_y,		0.0125 * frac, 	0.005 * frac, 	1.0,			0.9, 0.6, -.4, 0.5,	(this.age * 0.37 + 177) % 360, 	360);
	}
//*/
...
hc.Projectile.prototype._ = function()
{
}
