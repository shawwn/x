...
µ.WebGL_Rectangle_Light_and_Shadow2 = function(gl, cameras, textures, max_light_count)
{
	...
	this.number_of_active_lights = 0;
	...
	this.uniform_arrays = new Array(this.light_parameters.length);

	for  (var i = 0; i < this.light_parameters.length; i++)
	{
		this.uniform_arrays[i] = new Float32Array(this.max_light_count);
		for  (var j = 0; j < this.max_light_count; j++)
		{
			this.uniform_arrays[i][j] = 0;
		}
	}
...
*/}).toString().match(/[^]*\/\*([^]*)\*\/\}$/)[1],
	...
	'const int cMaxLightCount = ' + this.max_light_count + ';',

	'uniform float u_Light_Position_X['+this.max_light_count+'];',
	'uniform float u_Light_Position_Y['+this.max_light_count+'];',
	'uniform float u_Light_Position_Z['+this.max_light_count+'];',
	'uniform float u_Light_Range['+this.max_light_count+'];',
	'uniform float u_Light_Falloff['+this.max_light_count+'];',
	'uniform float u_Light_Range360['+this.max_light_count+'];',
	'uniform float u_Light_Color_R['+this.max_light_count+'];',
	'uniform float u_Light_Color_G['+this.max_light_count+'];',
	'uniform float u_Light_Color_B['+this.max_light_count+'];',
	'uniform float u_Light_Color_A['+this.max_light_count+'];',
	'uniform float u_Light_Cone['+this.max_light_count+'];',
	'uniform float u_Light_Direction['+this.max_light_count+'];',
...
vec4 do_the_bresenham_step(vec3 pos_start, vec4 light_color, vec3 pos_end, float step, float range)
{
	...
	for (int i = 0; i < 400; i++)
	{
		...
		if ((tx_depth[0] + tx_fluid[0]) > (fluid_height + uHeightTolerance))
		{
			...
			light_color[0] -= diff * 1.0;
			light_color[1] -= diff * 1.0;
			light_color[2] -= diff * 1.0;
			light_color[3] -= diff * 1.0;
		}
		...
		//if (tx[3] > 0.0)
		{
			light_color[0] -= (1.0 - tx[0]) * tx[3] / 5.0;
			light_color[1] -= (1.0 - tx[1]) * tx[3] / 5.0;
			light_color[2] -= (1.0 - tx[2]) * tx[3] / 5.0;
...
vec4 do_lights(vec2 screen_pos, float height)
{
	...
	for (int i = 0; i < cMaxLightCount; i++)
	{
		if (i < uLightCount)
		{
			light_pos_x = u_Light_Position_X[i];
			light_pos_y = u_Light_Position_Y[i];
			light_pos_z = u_Light_Position_Z[i];
			light_range = u_Light_Range[i];
			light_falloff = u_Light_Falloff[i];
			float light_direction = 360.0 - u_Light_Direction[i];
			float light_cone = (360.0 - u_Light_Cone[i] - 180.0) / 180.0;
			light_range360 = u_Light_Range360[i];
			...
			if (dist <= light_range)
			{
				...
				if (!(dist > light_range360 && is_outside_cone))
				{
					...
					light_factor = falloff * vec4(1.0, 1.0, 1.0, 1.0); // do_the_bresenham_step(light_pos3D, vec4(u_Light_Color_R[i], u_Light_Color_G[i], u_Light_Color_B[i], u_Light_Color_A[i]), screen_pos3D, uTextureStep * 0.04, (is_outside_cone ? light_range360 : light_range))
					light_color[0] += light_factor[0] * u_Light_Color_R[i] * u_Light_Color_A[i];
					light_color[1] += light_factor[1] * u_Light_Color_G[i] * u_Light_Color_A[i];
					light_color[2] += light_factor[2] * u_Light_Color_B[i] * u_Light_Color_A[i];
					light_color[3] += light_factor[3] * u_Light_Color_A[i];
				}
			}
		}
		else
...
(function () {/*
	...
	// debug
	//shadow1 = vec4(1.0, 1.0, 1.0, 1.0);

	if (shadows[0] > 0.0 || shadows[1] > 0.0 || shadows[2] > 0.0)
	{

		vec4 light_color = do_lights(pos, texDepthmap[0]);
	...
	//gl_FragColor = vec4(u_Light_Color_R[0], u_Light_Color_G[0], u_Light_Color_B[0], u_Light_Color_A[0]);
...
*/
	...
	for  (var i = 0; i < this.light_parameters.length; i++)
	{
		var parameter_name = this.light_parameters[i];
		this.program['u_' + parameter_name]		= gl.getUniformLocation(this.program, 'u_' + parameter_name);
	}
	...
	this.add_light = function(pos_x, pos_y, pos_z, range, range360, falloff, color_r, color_g, color_b, color_a, direction, cone)
	{
...
*/
		...
		this.uniform_arrays[0][this.number_of_active_lights] = pos_x;
		this.uniform_arrays[1][this.number_of_active_lights] = pos_y;
		this.uniform_arrays[2][this.number_of_active_lights] = pos_z;
		this.uniform_arrays[3][this.number_of_active_lights] = range;
		this.uniform_arrays[4][this.number_of_active_lights] = range360;
		this.uniform_arrays[5][this.number_of_active_lights] = falloff;
		this.uniform_arrays[6][this.number_of_active_lights] = color_r;
		this.uniform_arrays[7][this.number_of_active_lights] = color_g;
		this.uniform_arrays[8][this.number_of_active_lights] = color_b;
		this.uniform_arrays[9][this.number_of_active_lights] = color_a;
		this.uniform_arrays[10][this.number_of_active_lights] = direction;
		this.uniform_arrays[11][this.number_of_active_lights] = cone;

		// this gets reset when actually rendering, which is clunky but does for now
		this.number_of_active_lights++;
	}
	...
	this.draw = function(cam_id, texture_id, pos_x, pos_y, width, height)
	{
		...
		{
			console.log('rectangle_textured_light.draw(): unknown cam: ' + cam_id);
			return;
		}
...
/*
		console.log(
			this.uniform_arrays[0][0],
			this.uniform_arrays[1][0],
			this.uniform_arrays[2][0],
			this.uniform_arrays[3][0],
			this.uniform_arrays[4][0],
			this.uniform_arrays[5][0],
			this.uniform_arrays[6][0],
			this.uniform_arrays[7][0],
			this.uniform_arrays[8][0],
			this.uniform_arrays[9][0],
			this.uniform_arrays[10][0],
			this.uniform_arrays[11][0]
		);
*/
		//console.log(this.uniform_arrays[1]);
		//console.log(this.uniform_arrays[2]);

		//alert('bai');
	};
	this.flush_all = function(texture_depthmap_id, texture_fluid_map_id)
	{
		...
		for (var camera_id in cameras)
		{
			...
			{
				this.flush(camera_id, texture_id, texture_depthmap_id, texture_fluid_map_id);
			}
		}
	...
	this.flush = function(camera_id, texture_id, texture_depthmap_id, texture_fluid_map_id)
	{
		...
		gl.uniform1i(this.program.uLightCount, 		this.number_of_active_lights);
		...
		for  (var i = 0; i < this.light_parameters.length; i++)
		{
			gl.uniform1fv(this.program['u_' + this.light_parameters[i]], this.uniform_arrays[i]);
		}
		...
		gl.uniform1i(this.program.uSamplerFluidmap, 2);
