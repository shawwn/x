µ.Particles2D_uniform_arrays = function (gl, pDefs, particle_count, stop_count)
{
	this.stop_count = stop_count;
	this.pdef_parameters = [
		'_Color_r',
		'_Color_g',
		'_Color_b',
		'_Color_a',
	...
	this.pdef_stops_n_values = ['value_0'];
	for (var i = 0; i < this.stop_count - 1; i++)
	{
		this.pdef_stops_n_values.push('value_' + (i + 1));
		if (i > 0)
		{
			this.pdef_stops_n_values.push('stop_' + (i - 1));
		}
	}
	this.pdef_count = 0;
	for (var i in pDefs)
	...
	µ.log('[Particles2D_uniform_arrays] ' + this.pdef_count + ' particle definitions, ' + this.stop_count + ' stops', µ.LOGLEVEL_VERBOSE);
...
µ.Particles2D_uniform_arrays.prototype.add_particle = function (
	now,
	pos_x,
	pos_y,
	size_factor, 
...
µ.Particles2D_uniform_arrays.prototype.spawn = function (
	now,
	count,
	pos_x,
	pos_y,
	...
	)
{
	if (count === 0)
		return;
	var
		distance_x = pos2_x - pos_x,
		distance_y = pos2_y - pos_y,
		step_size = 1.0 / count,
		_180_over_pi = Math.PI / 180;
	for (var i = 0; i < count; ++i)
	{
		var
			angle_vel2 = vary_angle_vel ? angle_vel - vary_angle_vel / 2 + Math.random()*vary_angle_vel : angle_vel,
			angle2 = vary_angle ? angle - vary_angle / 2 + Math.random()*vary_angle : angle;
		angle2 = (360 + angle2) * _180_over_pi;
		this.add_particle(
			now + (vary_birthdate ? - (vary_birthdate / 2) + Math.random() * vary_birthdate : 0),
...
µ.Particles2D_uniform_arrays.prototype.draw = function(now, gl, camera)
{
