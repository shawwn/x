µ.WebGL_Rectangle_Scent = function(gl, cameras, textures)
{
...
void main()
{
	...
	//vec4 texColor = texture2D(uSampler, vec2(uTextureStep2 + pixel_x * uTextureStep * 1.0, uTextureStep2 + pixel_y * uTextureStep * 1.0));

	vec4 texColor = texture2D(uSampler, vec2(vTexPosition[0] * vTexScale[0],vTexPosition[1] * vTexScale[0]));

	float val = texColor[0] * uFactor1 + texColor[1] * uFactor2 + texColor[2] * uFactor3 + texColor[3] * uFactor4;

	//gl_FragColor[0] = 1.0 - mod(log(1.0 / texColor[0]) * 0.13, 1.0);
	//gl_FragColor[1] = mod(log(1.0 / texColor[1]) * 0.03, 1.0);
	//gl_FragColor[2] = mod(log(1.0 / texColor[2]) * 0.03, 1.0);

	if (val > 1.0)
	{
		//gl_FragColor = HSLA_to_RGBA(mod(pow(val, 2.0) * 360.0, 360.0), 0.99, 0.5, 0.99);

		if (mod((gl_FragCoord[0] * log(val) + gl_FragCoord[1] * val), 3.0) > 1.0)
		{
			gl_FragColor = HSLA_to_RGBA(mod(log(val * val) * 72.0, 360.0), 0.99, 0.5, 0.99);
		}
		else
		{
			gl_FragColor[0] = mod(log(val), 1.0);
			gl_FragColor[1] = mod(log(val * val), 1.0);
			gl_FragColor[2] = mod((1.0 / val), 1.0);
		}
	}
	else if (val > 0.0)
	{
		gl_FragColor = HSLA_to_RGBA(360.0 - mod(log(val) * 10.0, 360.0), 0.99, 0.45, 0.99);
	}
	else if (val < 0.0)
	{
		if (mod((gl_FragCoord[0] * gl_FragCoord[1]), mod(log(-val) * .0001, 8.0)) > 4.0)
...
//			gl_FragColor[2] = 0.0;
		...
		{
			gl_FragColor = HSLA_to_RGBA(360.0 + mod(log(-val) * 7.0, 360.0), 0.99, 0.45, 0.99);
//			gl_FragColor[0] = 0.5;
//			gl_FragColor[1] = 0.0;
//			gl_FragColor[2] = 1.0;
		}
...
//	gl_FragColor[0] = texColor[0];
//	gl_FragColor[1] = texColor[1];
//	gl_FragColor[2] = texColor[2];
...
*/
	...
	this.program.uFactor1			= gl.getUniformLocation(this.program, "uFactor1");
	this.program.uFactor2			= gl.getUniformLocation(this.program, "uFactor2");
	this.program.uFactor3			= gl.getUniformLocation(this.program, "uFactor3");
	this.program.uFactor4			= gl.getUniformLocation(this.program, "uFactor4");
	...
	this.flush_all = function(factor1, factor2, factor3, factor4)
	{
		...
		for (var camera_id in cameras)
		{
			...
			{
				this.flush(camera_id, texture_id, factor1, factor2, factor3, factor4);
			}
		}
	...
	this.flush = function(camera_id, texture_id, factor1, factor2, factor3, factor4)
	{
		...
		gl.uniform1f(this.program.uFactor1, factor1);
		gl.uniform1f(this.program.uFactor2, factor2);
		gl.uniform1f(this.program.uFactor3, factor3);
		gl.uniform1f(this.program.uFactor4, factor4);
