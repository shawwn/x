µ.WebGL_Rectangle_Fluids = function(gl, cameras, textures)
{
...
void main()
{
	...
	float alpha = min(0.0000001, texColor[0]) * 10000000.0 * 0.8 + (1.0 - ((1.0 - texColor[0]) * (1.0 - texColor[0]) * (1.0 - texColor[0]))) * 0.2;
	...
	hue += mod((vTexPosition[0] * 36.0 * (1.0 - texColor[0]) + vTexPosition[1] * 4144623.0 * texColor[0]), 100.0) / 10.0;
	hue += mod((vTexPosition[0] * 10.0 * texColor[0] + vTexPosition[1] * 623157.0 * (1.0 - texColor[0])), 100.0) / 10.0;

	hue -= 20.0 * mod((vTexPosition[0] * 31000.0 * (texColor[0]) + vTexPosition[1] * 41000.0 * texColor[0]), 250.0) / 250.0;

	//if (mod((vTexPosition[0] * 31000.0 * (1.0 - texColor[0]) + vTexPosition[1] * 41000.0 * texColor[0]), 500.0) > 250.0)
	{
		//hue = 200.0 + 50.0 * texColor[0];
	}

	//if (mod((vTexPosition[0] * 51000.0 * texColor[0] + vTexPosition[1] * 61000.0 * (1.0 - texColor[0])), 200.0) > 250.0)
	{
		//hue += 10.0;
	}
	
	hue = log(texColor[0]) * 2000.0;


	gl_FragColor = HSLA_to_RGBA(mod(hue, 360.0), sat, 0.5 - vColor[3] * 0.2, alpha * vColor[3]);
