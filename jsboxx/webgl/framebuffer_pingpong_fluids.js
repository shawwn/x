...
µ.WebGL_Framebuffer_Pingpong_Fluids = function(c, gl, cameras, textures, texture_size, fragment_shader_fragment)
{
...
float flow(vec4 texColor, vec4 texDepth, float pixel_x, float pixel_y, float direction_x, float direction_y)
{
		...
		)
	{
		...
		texColor_neighbour = texture2D(uSampler, tex_pos);
		if (texWall_neighbour[3] == 0.0)
		{
			diff = (texDepth_neighbour[0] + texColor_neighbour[channel]) - (texDepth[0] + texColor[channel]);
			// if neighbour is higher
			//{ if (diff > 0.0)
			{
				//{ flow += min(diff / 2.0, texColor_neighbour[channel]);
				flow += min(ceil(max(diff, 0.0)), 1.0) * min(diff / 2.0, texColor_neighbour[channel]);
			}
		}
	}
	if (
		...
		)
	{
		...
		if (texWall_neighbour[3] == 0.0)
		{
			...
			//{ if (diff < 0.0)
			{
				//{ flow -= min(- diff / 2.0, texColor[channel]);
				flow -= -1.0 * max(floor(min(diff, 0.0)), -1.0) * min(- diff / 2.0, texColor[channel]);
			}
		}
	}
	return texColor[channel] + flow;
...
(function () {/*
	...
	float old_value = texColor[channel];
	...
	if (texWall[3] == 0.0)
	{
		...
		texColor[channel] = flow(texColor, texDepth, pixel_x, pixel_y, direction_x, direction_y);
	}

	//float diff = texColor[channel] - old_value;
	
	// "age"
	if (texColor[channel] > 0.0)
	{
		if (texColor[1] < 1000000000000000000.0)
		{
			texColor[1] += 1.0;
		}
	}
	...
	// evaporate
	//if (texColor[channel] < 0.004 && texColor[1] > 100.0)
	{
		//texColor[channel] -= (0.0000002);
	}

	if (texColor[channel] < 0.0)
	{
		texColor[channel] = 0.0;
	}

	texColor[0] = max(0.0, texColor[0]);
	texColor[1] = max(0.0, texColor[1]);
	texColor[2] = max(0.0, texColor[2]);
	texColor[3] = max(0.0, texColor[3]);

	// rain :)
	// texColor[channel] += (0.000002);
...
*/
	...
	this.process_steps = function(step_count, depthmap_texture_id, wallmap_texture_id)
	{
		gl.useProgram(this.program);
